...
public class ClassInfo extends JFrame{
	...
	public ClassInfo() {
		...
		for(int i=0;i<methods.length;i++)
			System.out.println("현재 객체가 보유한 메서드는 "+ methods[i].getName());
		
		//사실은 메서드명을 조사하려고 그런게 아님...
		//Class 클래스를 이용하면 해당 클래스의 환경정보들도 접근할 수 있다..
		//따라서 우리는 클래스 패스를 조사해서 우리가 사용중인 res라는 패키지의 경로를 조사할 거임..
		...
		URL url = myClass.getClassLoader().getResource("res/github.png");
		//url은 자원의 위치를 말한다!! (http, www 가 붙으면 웹상의 자원의 위치)
